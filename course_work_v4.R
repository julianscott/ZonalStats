...
# jascott@usda.gov
# TODO: load nlcd class table to github
# prepare burn severity module for spillover


# First, here is a list of the required libraries
packages <- c("tmap","sf","tidyverse","sp","nhdplusTools","raster","FedData","prism","RColorBrewer","data.table","rgdal")
...
# Because I want to perform zonal statistics on the polygons in the catchment shapefile, its good practice to download the raster data with a slightly larger footprint than the polygon data, so there aren't issues of NoData cells. 
...
area_NED <- get_ned(template = as_Spatial(buf_catchment),
                    extraction.dir = ".\\EXTRACTIONS\\NED\\",
                     ...
                     res = "1",
                     force.redo = F)
# area_NED <- raster(".\\EXTRACTIONS\\30meter\\NED\\30meter_NED_1_bu.tif")
names(area_NED) <- "elev"
...
# area_NLCD <- raster(".\\EXTRACTIONS\\NLCD\\NLCD_2016_Land_Cover_L48_bu.tif")
...
NLCD_classes_results <- NLCD_classes %>% 
  ...
  select(-n,-total_cells_per_catchment) %>% 
  # Pivot the table so that each cover class has its own column
  pivot_wider(names_from = NLCD_2016_Land_Cover_L48,
...
NLCD_classes_results <- NLCD_classes_results %>% 
  ...
  select(catchment,catchmentID,Class,perc_catchm) %>% 
